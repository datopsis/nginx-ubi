...
| ClickHouse HTTP proxying profile | Preview/unqualified | Method and body bounds, credential and query-text exclusion from logs, exception-code reporting, and bounded temporary storage are tested against a stand-in; interoperation with a real ClickHouse server is not qualified. |
| Dynamic upstream resolution profile | Preview/unqualified | Per-request re-resolution and recovery from a replaced endpoint without a reload are tested; it has no balancing, failure tracking, failover, or connection reuse, and resolver failure modes are not qualified. |
